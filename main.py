...
class ZenBankApp(ctk.CTk):
    ...
    def _carregar_assets(self):
        ...
        try:
            ...
            assets['menu'] = load_icon("menu.png")
    ...
    def tela_splash(self):
        ...
        self.canvas.create_text(cx, cy + 60, text="Fique Zen com suas finanças", fill=COR_PRIMARIA, font=("Arial", 14), tags="ui")

        bar = ctk.CTkProgressBar(self.canvas, width=220, progress_color=COR_PRIMARIA, fg_color="#333", bg_color="#0a1a10")
        self.add_widget(0.5, 0.65, bar)
        bar.set(0)
        bar.start()
        self.after(2500, self.tela_login)
    ...
    def tela_login(self):
        ...
        self.ent_cpf = ctk.CTkEntry(self.canvas, placeholder_text="👤 CPF (Números)", width=300, height=50, fg_color=COR_INPUTS,
                                    border_width=0, corner_radius=12, text_color="white", bg_color="#0a1a10")
        ...
        self.ent_senha = ctk.CTkEntry(self.canvas, placeholder_text="🔐 Senha", show="*", width=300, height=50,
                                      fg_color=COR_INPUTS, border_width=0, corner_radius=12, text_color="white", bg_color="#0a1a10")
        self.add_widget(0.5, 0.52, self.ent_senha)

        self.lbl_erro = ctk.CTkLabel(self.canvas, text="", text_color=COR_ERRO, fg_color="transparent", bg_color="#0a1a10")
    ...
    def tela_cadastro(self):
        ...
        btn_voltar = ctk.CTkButton(self.canvas, text="⬅️ Voltar", width=80, fg_color="transparent", text_color="white",
        ...
        y_start, gap = 0.22, 0.08

        self.ent_nome = ctk.CTkEntry(self.canvas, placeholder_text="📝 Nome Completo", width=300, height=45, fg_color=COR_INPUTS, border_width=0, corner_radius=10, bg_color="#0a1a10")
        self.add_widget(0.5, y_start, self.ent_nome)

        self.ent_cpf_cad = ctk.CTkEntry(self.canvas, placeholder_text="👤 CPF (números)", width=300, height=45, fg_color=COR_INPUTS, border_width=0, corner_radius=10, bg_color="#0a1a10")
        self.add_widget(0.5, y_start + gap, self.ent_cpf_cad)

        self.ent_s1 = ctk.CTkEntry(self.canvas, placeholder_text="🔐 Senha (min 8 chars)", show="*", width=300, height=45, fg_color=COR_INPUTS, border_width=0, corner_radius=10, bg_color="#0a1a10")
        self.add_widget(0.5, y_start + gap * 2, self.ent_s1)

        self.ent_s2 = ctk.CTkEntry(self.canvas, placeholder_text="🔐 Repetir Senha", show="*", width=300, height=45, fg_color=COR_INPUTS, border_width=0, corner_radius=10, bg_color="#0a1a10")
        self.add_widget(0.5, y_start + gap * 3, self.ent_s2)

        self.tipo_var = ctk.StringVar(value="1")
        sw = ctk.CTkSwitch(self.canvas, text="É Conta Poupança?", variable=self.tipo_var, onvalue="2", offvalue="1", progress_color=COR_PRIMARIA, text_color="white")
        self.add_widget(0.5, y_start + gap * 4, sw)

        self.lbl_feed = ctk.CTkLabel(self.canvas, text="", font=("Arial", 12), fg_color="transparent", bg_color="#0a1a10")
        self.add_widget(0.5, y_start + gap * 4.7, self.lbl_feed)

        btn_cad = ctk.CTkButton(self.canvas, text="FINALIZAR", width=300, height=50, fg_color=COR_PRIMARIA, text_color="black", font=("Arial", 15, "bold"), bg_color="#0a1a10", command=self.confirmar)
    ...
    def tela_dashboard(self):
        ...
        self.canvas.create_text(30, 80, text=f"Olá, {nome} 👋", fill="white", font=("Arial", 20, "bold"), anchor="w", tags="ui")

        # Agência e Conta
        dados_conta = f"Ag: {self.conta_logada.agencia} | CC: {self.conta_logada.numero_conta}"
        self.canvas.create_text(30, 110, text=dados_conta, fill="gray", font=("Arial", 12), anchor="w", tags="ui")
        ...
        botoes = [
            ...
            ("Cartões", "card", self.em_breve),
            ("Investimentos", "saque", self.em_breve),
    ...
    def tela_configuracoes(self):
        ...
        btn_voltar = ctk.CTkButton(self.canvas, text="⬅️ Voltar", width=80, fg_color="transparent", text_color="white",
                                   command=self.tela_dashboard, anchor="w", bg_color="#0a1a10")
        self.add_widget(0.1, 0.05, btn_voltar)
    ...
    def tela_editar_perfil(self):
        ...
        btn_voltar = ctk.CTkButton(self.canvas, text="⬅️ Voltar", width=80, fg_color="transparent", text_color="white", command=self.tela_configuracoes, anchor="w", bg_color="#0a1a10")
        self.add_widget(0.1, 0.05, btn_voltar)
        ...
        self.ent_nome_edit = ctk.CTkEntry(self.canvas, width=300, height=40, fg_color=COR_INPUTS, border_width=0,
                                          corner_radius=10, bg_color="#0a1a10")
        ...
        self.ent_cpf_edit = ctk.CTkEntry(self.canvas, width=300, height=40, fg_color="#111", border_width=0,
                                         corner_radius=10, state="readonly", bg_color="#0a1a10")
        ...
        self.lbl_feed_edit = ctk.CTkLabel(self.canvas, text="", font=("Arial", 12), fg_color="transparent", bg_color="#0a1a10")
        ...
        btn_salvar = ctk.CTkButton(self.canvas, text="SALVAR ALTERAÇÕES", width=300, height=50, fg_color=COR_PRIMARIA,
                                   text_color="black", font=("Arial", 15, "bold"), command=self.salvar_edicao, bg_color="#0a1a10")
    ...
    def tela_transacao(self, tipo):
        ...
        self.ent_val = ctk.CTkEntry(self.canvas, placeholder_text="R$ 0,00", width=300, height=50, fg_color=COR_INPUTS,
                                    border_width=0, font=("Arial", 20, "bold"), bg_color="#0a1a10")
        self.add_widget(0.5, y_pos, self.ent_val)
        self.lbl_msg = ctk.CTkLabel(self.canvas, text="", font=("Arial", 14, "bold"), fg_color="transparent", bg_color="#0a1a10")
    ...
    # --- LÓGICA AUXILIAR ---
    def em_breve(self):
        messagebox.showinfo("ZenBank", "Funcionalidade em desenvolvimento.\nAguarde novidades! 🚀")
    ...
    def confirmar(self):
        ...
        if not n or not c or not s1:
            return self.lbl_feed.configure(text="Preencha tudo", text_color=COR_ERRO)
        if s1 != s2:
            return self.lbl_feed.configure(text="Senhas não conferem", text_color=COR_ERRO)
        if not re.match(r"^(?=.*[a-z])(?=.*[A-Z])(?=.*[\W_]).{8,}$", s1):
            return self.lbl_feed.configure(text="Senha fraca", text_color=COR_ERRO)
